    ...
    def tmpl_add_comment_form(
        ...
        for (matching_value, (matching_key, data)) in CFG_WEBCOMMENT_EXTRA_CHECKBOX.items():
            if matching_value in get_fieldvalues(recID, matching_key):
                ...
                if extra_checkbox:
                    has_extra_checkbox = extra_checkbox, data.get('label')
                    break
        ...
        # be configured first
        extra_checkbox = ""
        if has_extra_checkbox and len(has_extra_checkbox) == 2:
            extra_checkbox = (
                "<label><input type='checkbox' name='extra_checkbox' "
                "value=''/> {0}</label>").format(has_extra_checkbox[1])
